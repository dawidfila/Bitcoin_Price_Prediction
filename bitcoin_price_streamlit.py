...
exchange_pair = st.text_input("Enter the exchange pair:",exchange_pair)

bit_coin_data = yf.download(exchange_pair, start, end)
...
def predict_future(no_of_days, prev_100):
    ...
    for i in range(int(no_of_days)):
        next_day = model.predict(np.array(prev_100)).tolist()
